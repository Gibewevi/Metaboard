  ...
  const AccountsComponent = () => {
    if (accounts.length > 0) {
      return accounts.map((account, key) => {
        return (
          <Account account_id={account.account_id} strategy={account.strategy} current_balance={account.current_balance} profit_and_loss={account.profit_and_loss} profit_and_loss_percent={`${account.profit_and_loss_percent}%`}
